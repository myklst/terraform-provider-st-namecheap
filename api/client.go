...
// Client is a NameCheap API client
type Client struct {
	baseURL  string
	key      string
	name     string
...
// rateLimitedTransport throttles API calls to NameCheap. It appears that
...
// NewClient constructs a new NameCheap API client or an error if the supplied
// input is invalid.
func NewClient(baseURL, key, name string, clinetIP string) (*Client, error) {
	...
	return &Client{
		...
		clientIP: strings.TrimSpace(clinetIP),
		client: &http.Client{
